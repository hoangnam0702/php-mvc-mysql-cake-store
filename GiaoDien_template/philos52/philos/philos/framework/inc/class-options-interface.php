	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			case 'textarea':
				...
				$val = stripslashes( $val );
				$output .= '<textarea id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" cols="' . $cols . '"'. 'rows=' . $rows . $placeholder . '>' . esc_textarea( $val ) . '</textarea>';
				break;
			...
			// Uploader
			case "upload":
				$output .= Nileforest_Framework_Media_Uploader::optionsframework_uploader( $value['id'], $val, null );
				break;

			// Typography
			case 'typography':
				unset( $font_size, $font_style, $font_face,$font_weight,$font_color );
			...
			case 'background':
				...
				$output .= Nileforest_Framework_Media_Uploader::optionsframework_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
